    ...
    public void render(GuiRenderer renderer) {
        ...
        for (int i = 0; i < getRow() * getColumn(); l++) {
            if (i < components.size() && l < components.size() && !(hideIfDisable && components.get(l) instanceof GuiButton button && !button.isEnable()))
                components.get(l).render(new GuiRenderer(renderer.getInventory(), new GuiRenderer(renderer.getInventory(), renderer, i, 1), components.get(l).getSlot(), components.get(l).getColumn()));
            else
